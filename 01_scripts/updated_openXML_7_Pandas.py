...
for i, row in enumerate(sheet['D4:F67']):
    ...
    row_dict = {
        'сomplexity': str(complexity),  # Добавляем значение из столбца C с дополнительным значением из B
    ...
    # Добавляем словарь в общий словарь с ключом docs_name
    id = len(data_dict) + 1
...
# Функция для замены формул
def replace_formula(expression):
    """
    Заменяет формулу Excel в формате:
    =IF(user_parametr_3=1,D5+E5+4,0)
    на формат:
    = IF ( user_parametr_3 == 1 , @2$parametr_1 + @2$parametr_2 + 4 , 0 )
    """
    pattern = r"=IF\(([^,]+)=([^,]+),([^,]+)\+([^,]+)\+(\d+),(\d+)\)"
    replacement = r"= IF ( \1 == \2 , @2$parametr_1 + @2$parametr_2 + \5 , \6 )"
    return re.sub(pattern, replacement, expression)

# Обработка словаря с формулами
data_dict = {key: replace_formula(value) if isinstance(value, str) else value
             for key, value in data_dict.items()}

# Пример вывода обработанных данных (можно закомментировать или удалить)
