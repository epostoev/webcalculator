...
# Читаем диапазон ячеек A5:A105 для значений docs_name
docs_name_values = [row[0].value for row in sheet['A5:A105']]  # Извлекаем значения из первого столбца каждой строки

# Обрабатываем объединенные ячейки в столбце A, чтобы распространить значение на все объединенные строки
...
with open('output.txt', 'w', encoding='utf-8') as file:
    for row in data_dict:
        row_str = ';'.join([row['docs_name'], row['parametr_1'], row['parametr_2'], row['parametr_3']])
        file.write(row_str + '\n')
